...
for num in range(83):
    predict_list.append(pickle.load(open('Data/Transformer_17/predict_list_main_17_'+ str(num) +'.pkl', 'rb')))
'''
for num in range(40):
    predict_list.append(pickle.load(open('Data/Test_14/predict_list_test_14_'+ str(num) +'.pkl', 'rb')))
'''
...
for i in Battery_list:
    ...
    plt.savefig('./Data/Transformer_17/SOH_'+ str(i) +'.png')
    plt.show()
    count += 1
print(count)
'''
...
for i in Battery_list:
    ...
    plt.plot(train_batch[i]['summary']['cycle'], train_batch[i]['summary']['QD']/1.1, ls = '-', label='Real') 
    x = np.arange(0, len(train_batch[i]['summary']['cycle']))
    plt.plot(x[start_index:],np.array(predict_list[count])/1.1, ls = 'dotted', label='Prediction')
    plt.plot(train_batch[i]['summary']['cycle'], [0.8]*len(train_batch[i]['summary']['cycle']), c ='r', ls = '-.', label='Failure threshold')  
    plt.xlabel('Cycle Number')
    plt.ylabel('SOH(%)')
    plt.title('SOH Prediction')
    plt.legend(loc=0,ncol=3)
    plt.savefig('./Data/Test_14/SOH_'+ str(i) +'.png')
    plt.show()
    count += 1
print(count)
'''
