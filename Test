...
def test(lr=0.01, feature_size=8, hidden_dim=32, num_layers=1, nhead=8, weight_decay=0.0, EPOCH=1000, seed=0, 
    ...
    for i in range(40):
        ...
        np.savetxt('Data/Test_15/train_data_15_' + str(i) + '.csv', train_data, delimiter=',')
        np.savetxt('Data/Test_15/test_data_15_' + str(i) + '.csv', test_data, delimiter=',')
        ...
        if is_load_weights: 
            if torch.__version__.split('+')[0] >= '1.6.0':
                ...
                model = torch.load('Data/Transformer15.pkl')
            else:
        ...
        if torch.__version__.split('+')[0] >= '1.6.0':
            torch.save(model.state_dict(), 'Data/Test_15_seed'+str(seed)+'.pth')
        else:
            torch.save(model.state_dict(), 'Data/Test_15.pth', _use_new_zipfile_serialization=False)        
        '''
        '''
        ...
        np.savetxt('Data/Test_15/predict_list_test_15_' + str(i) + '.csv', y_, delimiter=',')   
        with open('Data/Test_15/predict_list_test_15_' + str(i) + '.pkl', 'wb') as fp:
            pickle.dump(y_, fp)
...
with open('Data/Test_15/predict_list_15.pkl', 'wb') as fp:
            pickle.dump(result_list, fp)
print(np.array(score_list))
np.savetxt('Data/Test_15/score_list_test_15_.csv', score_list, delimiter=',')
for s in score_list:
...
np.savetxt('Data/Test_15/Test_15_Score.csv', mean_values, delimiter=',')
